...
sigp$cx <- 1:lyx
write.table(sigp,paste(Sys.getenv("resc"),"_low.sig.pValue_",alpha,sep=""),row.names=F,col.names=F,quote=F,sep=" ")
write.table(sigw,paste(Sys.getenv("resc"),"_low.sig.weight_",alpha,sep=""),row.names=F,col.names=F,quote=F,sep=" ")
